        ...
        <div role="tabpanel" class="tab-pane active" id="selling">
            <table class="table table-condensed table-bordered">
                <tr>
                    ...
                    <td class='align-middle text-center' colspan='4'><b>Target Oli (Selling)</b></td>
                ...
                <tr v-if='filtered_target_salesman_oils.length > 0'>
                    <td class='text-right' colspan='9'>Total Target (Selling)</td>
                    <td class='text-right'>
        ...
        <div role="tabpanel" class="tab-pane" id="cost">
            <table class="table table-condensed table-bordered">
                <tr>
                    ...
                    <td class='align-middle text-center' rowspan='3'>Alamat</td>
                    <td class='align-middle text-center' rowspan='3'>Kota / Kabupaten</td>
                    <td class='align-middle text-center' colspan='4'><b>Target Oli (Cost)</b></td>
                    <td class='align-middle text-center' colspan='2' rowspan='2'>Total</td>
                    <td v-if='mode != "detail"' rowspan='3' width='3%'></td>
                </tr>
                <tr>
                ...
                <tr v-if="target_salesman_oils.length > 0" v-for="(target_salesman_oil, index) of filtered_target_salesman_oils">
                    ...
                    <td class='align-middle'>{{ target_salesman_oil.kabupaten }}</td>
                    <td class='text-right'>
                        <vue-numeric :read-only='mode == "detail"' class="form-control" currency="Rp" separator="." v-model='target_salesman_oil.cost_amount_engine_oil'></vue-numeric>
                    </td>
                    <td class='text-center'>
                        <vue-numeric :read-only='mode == "detail"' class="form-control" separator="." v-model='target_salesman_oil.cost_botol_engine_oil'></vue-numeric>
                    </td>
                    <td class='text-right'>
                        <vue-numeric :read-only='mode == "detail"' class="form-control" currency="Rp" separator="." v-model='target_salesman_oil.cost_amount_gear_oil'></vue-numeric>
                    </td>
                    <td class='text-center'>
                        <vue-numeric :read-only='mode == "detail"' class="form-control" separator="." v-model='target_salesman_oil.cost_botol_gear_oil'></vue-numeric>
                    </td>
                    <td class='text-right'>
                        <vue-numeric :read-only='mode == "detail"' read-only class="form-control" currency='Rp' separator="." v-model='cost_hitung_amount_target_salesman_oil(target_salesman_oil)'></vue-numeric>
                    </td>
                    <td class='text-center'>
                        <vue-numeric read-only class="form-control" separator="." v-model='cost_hitung_botol_target_salesman_oil(target_salesman_oil)'></vue-numeric>
                    </td>
                    <td v-if='mode != "detail"'>
                        <button class="btn btn-flat btn-sm btn-danger" @click.prevent='hapus_target_salesman_oils(index)'><i class="fa fa-trash-o"></i></button>
                    </td>
                ...
                <tr v-if='filtered_target_salesman_oils.length > 0'>
                    <td class='text-right' colspan='9'>Total Target (Cost)</td>
                    <td class='text-right'>
                        <vue-numeric class="form-control" v-model='cost_target_salesman_channel' currency='Rp' separator='.' read-only></vue-numeric>
                    </td>
                    <td class='text-center'>
                        <vue-numeric class="form-control" v-model='cost_total_botol' separator='.' read-only></vue-numeric>
                    </td>
                    <td v-if='mode != "detail"'></td>
                </tr>
                <tr v-if="filtered_target_salesman_oils.length < 1">
