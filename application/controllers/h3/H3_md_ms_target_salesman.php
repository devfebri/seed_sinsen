...
class H3_md_ms_target_salesman extends Honda_Controller {
	...
	{		
		...
		$data = $this->input->post([
			'id_salesman', 'start_date', 'end_date', 'jenis_target_salesman',
			'target_salesman_global', 'target_salesman_global_cost',
			'cost_target_salesman_global', 'cost_target_salesman_global_cost',
			'target_salesman_dus_global', 'target_salesman_channel', 'target_salesman_dus_channel',
			'cost_target_salesman_dus_global', 'cost_target_salesman_channel', 'cost_target_salesman_dus_channel',
		]);
		...
		if(count($target_salesman_parts) > 0){	
			foreach ($target_salesman_parts as $each) {
				$data = [
					...
					'cost_target_part' => $each['cost_target_part'],
					'global' => $each['global'],
				];
		...
		if(count($target_salesman_oils) > 0){
			$data = $this->getOnly([
				...
				'cost_amount_engine_oil', 'cost_botol_engine_oil',
				'cost_amount_gear_oil', 'cost_botol_gear_oil', 'cost_total_amount', 'cost_total_botol'
		...
		if(count($target_salesman_acc) > 0){
			$data = $this->getOnly([
				'id_dealer', 'target_acc','cost_target_acc'
			], $target_salesman_acc, [
				'id_target_salesman' => $id_target_salesman
			]);
		...
		if(count($target_salesman_apparel) > 0){
			$data = $this->getOnly([
				'id_dealer', 'target_apparel','cost_target_apparel'
