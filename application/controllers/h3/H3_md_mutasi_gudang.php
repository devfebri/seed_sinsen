...
class H3_md_mutasi_gudang extends Honda_Controller {
	...
	public function save(){
		...
		$data = array_merge($this->input->post([
			'id_part', 'id_gudang_awal', 'id_lokasi_awal', 'id_lokasi_tujuan',
			'id_gudang_tujuan', 'qty',
		]), [
	...
	public function riwayat_mutasi(){
		$data['mode']	='riwayat_mutasi';
		$data['set']	='riwayat_mutasi';
		$data['header']	= $this->db
			->select('mg.id_part')
			->select('p.nama_part')
			->from('tr_h3_md_mutasi_gudang as mg')
			->join('ms_part as p', 'mg.id_part=p.id_part')
			->where('mg.id_part', $this->input->get('id_part'))
			->get()
			->row_array();
		$this->template($data);
	}
