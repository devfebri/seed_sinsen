...
class Part extends Honda_Controller {
	...
	public function store_upload_fix_part(){
		...
		if (!$this->upload->do_upload('file')){
			...
			if(count($data) > 0 AND $this->input->post('force') == 0){
				$label_kode_part = '';
				$loop = 1;
				foreach($data as $row){
					if($loop == 50){
					...
					$label_kode_part .= ', ' . $row['id_part'];
					$loop++;
				}
				
				$sisa_label_kode_part = count($data) - $loop;
				$label_kode_part = substr($label_kode_part, 2);
				// var_dump($sisa_label_kode_part);
				// die;
				...
				$message = sprintf('Terdapat %s part yang akan diupdate. Berikut adalah list kode part, antara lain: %s', count($data), $label_kode_part);
			...
			if(count($data) > 0){
				...
				foreach($data as $row3){
					$this->db->set('p.fix', 1)
					->set('p.kelipatan_order_dk',$row3['kelipatan_order_dk'])
					->set('p.kelipatan_order_dm',$row3['kelipatan_order_dm'])
					->set('p.kelipatan_order_db',$row3['kelipatan_order_db'])
					->where('p.id_part', $row3['id_part'])
					->update('ms_part as p');
				}
			}
			$this->db->trans_complete();
	...
	public function read_excel_fix_part($filename){
        ...
        for ($row = 2; $row <= $highestRow; $row++){
			//  Read a row of data into an array
			$rowDataIdPart = $sheet->rangeToArray("B$row", NULL, TRUE, FALSE)[0];
			$rowDataDeskripsi = $sheet->rangeToArray("C$row", NULL, TRUE, FALSE)[0];
			$rowDataKelipatanOrderDk = $sheet->rangeToArray("D$row", NULL, TRUE, FALSE)[0];
			$rowDataKelipatanOrderDm = $sheet->rangeToArray("E$row", NULL, TRUE, FALSE)[0];
			$rowDataKelipatanOrderDb = $sheet->rangeToArray("F$row", NULL, TRUE, FALSE)[0];
			// $rowData = $sheet->rangeToArray('A' . $row . ':' . $highestColumn . $row, NULL, TRUE, FALSE)[0];

			if($rowDataIdPart[0] == null || $rowDataIdPart[0] == '') break;
			
			$data[$key]['id_part'] = $rowDataIdPart[0];
			$data[$key]['deskripsi'] = $rowDataDeskripsi[0];
			$data[$key]['kelipatan_order_dk'] = $rowDataKelipatanOrderDk[0];
			$data[$key]['kelipatan_order_dm'] = $rowDataKelipatanOrderDm[0];
			$data[$key]['kelipatan_order_db'] = $rowDataKelipatanOrderDb[0];
			$key++;
		}
		return $data;
	}
