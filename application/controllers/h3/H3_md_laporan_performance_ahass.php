...
class H3_md_laporan_performance_ahass extends CI_Controller{
	...
	public function downloadReport()
	{
		...
		if ($type == 'laporan_performance_ahass') {
			...
			$data['laporan_sales_by_channel_target_part_m_selling'] = $laporan_sales_by_channel_target_part_m_selling = $this->laporan_performance->laporan_sales_by_channel_target_part_m_selling($id_dealer, $start_date, $end_date, $type);
			$data['laporan_sales_by_channel_target_part_m1_selling'] = $laporan_sales_by_channel_target_part_m1_selling = $this->laporan_performance->laporan_sales_by_channel_target_part_m1_selling($id_dealer, $start_date, $end_date, $type);
			$data['laporan_sales_by_channel_target_part_m_cost'] = $laporan_sales_by_channel_target_part_m_cost = $this->laporan_performance->laporan_sales_by_channel_target_part_m_cost($id_dealer, $start_date, $end_date, $type);
			$data['laporan_sales_by_channel_target_part_m1_cost'] = $laporan_sales_by_channel_target_part_m1_cost = $this->laporan_performance->laporan_sales_by_channel_target_part_m1_cost($id_dealer, $start_date, $end_date, $type);

			$data['laporan_sales_by_channel_target_oli_m_cost'] = $laporan_sales_by_channel_target_oli_m_cost = $this->laporan_performance->laporan_sales_by_channel_target_oli_m_cost($id_dealer, $start_date, $end_date, $type);
			$data['laporan_sales_by_channel_target_oli_m1_cost'] = $laporan_sales_by_channel_target_oli_m1_cost = $this->laporan_performance->laporan_sales_by_channel_target_oli_m1_cost($id_dealer, $start_date, $end_date, $type);
			$data['laporan_sales_by_channel_target_oli_m_selling'] = $laporan_sales_by_channel_target_oli_m_selling = $this->laporan_performance->laporan_sales_by_channel_target_oli_m_selling($id_dealer, $start_date, $end_date, $type);
			$data['laporan_sales_by_channel_target_oli_m1_selling'] = $laporan_sales_by_channel_target_oli_m1_selling = $this->laporan_performance->laporan_sales_by_channel_target_oli_m1_selling($id_dealer, $start_date, $end_date, $type);
		...
		} elseif ($type == 'laporan_htl_dph_md') {
			$data['laporan_htl_dph_md'] = $laporan_htl_dph_md = $this->laporan_performance->laporan_htl_dph_md($id_dealer, $start_date, $end_date, $type);
			// $data_gr=array();
				
			// foreach($data['laporan_htl_dph_md']->result() as $row){
			// 	$grp=$this->db->query("
			// 	SELECT (".$row->kuantitas."-sum(grp.qty)) as bo, (select GROUP_CONCAT(DATE_FORMAT(gr.created_at ,'%d/%m/%Y')) from tr_h3_dealer_good_receipt gr join tr_h3_dealer_good_receipt_parts grp on grp.id_good_receipt=gr.id_good_receipt where gr.nomor_po='" . $row->po_id . "' and grp.id_part_int =".$row->id_part_int." 
			...
			// 	and grp.id_part_int=".$row->id_part_int. "
			// 	WHERE gr.nomor_po = '".$row->po_id."'
			// 	HAVING bo=0")->row();
				
			// 	$data_gr[]=$grp;
			// 	$data['data_bo']=$data_gr;
			// }
			// var_dump($data);
			// die;
			if ($_POST['process'] == 'excel') {
				$this->load->view("h3/laporan/temp_laporan_htl_dph_md", $data);
			}
		} elseif ($type == 'laporan_htl_order_ke_ahm') {
			$data['laporan_htl_order_ke_ahm'] = $laporan_htl_order_ke_ahm = $this->laporan_performance->laporan_htl_order_ke_ahm($id_dealer, $start_date, $end_date, $type);
			// $data_po = array();
			// foreach ($data['laporan_htl_order_ke_ahm']->result() as $row) {
				
			// 	if ($row->proses_ahm == '1') {
			// 		$customer_query = $this->db->query("SELECT DATE_FORMAT(hewh.tgl_ps ,'%d/%m/%Y') as tgl_packing_sheet_ahm,
			// 		DATE_FORMAT(hewh.tgl_penerimaan_md ,'%d/%m/%Y') as tgl_parts_diterima_md
			// 		FROM tr_h3_md_history_estimasi_waktu_hotline hewh
			// 		WHERE hewh.id_purchase_order='$row->id_purchase_order' and hewh.id_part='$row->id_part'
			// 		")->row();
			// 		$data_po[] = $customer_query;
			// 	} else {
			// 		$customer_query = $this->db->query("SELECT '' as tgl_packing_sheet_ahm, '' as tgl_parts_diterima_md, ''
			// 		")->row();
			// 		$data_po[] = $customer_query;
			// 	}
			// }
			// $data['data_po'] = $data_po;
			if ($_POST['process'] == 'excel') {
				$this->load->view("h3/laporan/temp_laporan_htl_order_ke_ahm", $data);
			}
		}elseif ($type == 'laporan_bo_htl_monitoring') {
			$data['laporan_bo_htl_monitoring'] = $laporan_bo_htl_monitoring = $this->laporan_performance->laporan_bo_htl_monitoring($id_dealer, $start_date, $end_date, $type);
			
			if ($_POST['process'] == 'excel') {
				$this->load->view("h3/laporan/temp_laporan_bo_htl_monitoring", $data);
			}
		}
	}
}
