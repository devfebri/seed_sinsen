...
class H3_md_pemenuhan_po_urgent_dealer extends Honda_Controller
{
	...
	public function detail()
	{
		...
		$data['parts'] = array_map(function ($row) {
			...
			// $row['qty_belum_terpenuhi'] = 5;
			return $row;
		}, $this->db->get()->result_array());

		$data['kel_parts']=$this->db->query('SELECT `skp`.`produk`, SUM(ifnull(ppdd.qty_pemenuhan, 0)) as hasil
			FROM `tr_h3_dealer_purchase_order_parts` as `pop` 
			LEFT JOIN `tr_h3_md_pemenuhan_po_dari_dealer` as `ppdd` ON (`ppdd`.`id_part` = `pop`.`id_part` and `ppdd`.`po_id` = pop.po_id) 
			JOIN `ms_part` as `p` ON `p`.`id_part` = `pop`.`id_part` 
			JOIN `ms_h3_md_setting_kelompok_produk` as `skp` ON `skp`.`id_kelompok_part`=`p`.`kelompok_part` 
			WHERE `pop`.`po_id` = "'.$id.'"
			GROUP BY skp.produk')->result_array();
	...
	public function edit()
	{
		...
		$data['kel_parts'] = $this->db->select('skp.produk')
		->distinct()
		->from('tr_h3_dealer_purchase_order_parts as pop')
		->join('tr_h3_md_pemenuhan_po_dari_dealer as ppdd', '(ppdd.id_part = pop.id_part and ppdd.po_id = pop.po_id)', 'left')
		->join('ms_part as p', 'p.id_part = pop.id_part')
