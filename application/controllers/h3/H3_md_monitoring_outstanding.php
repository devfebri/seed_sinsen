...
class H3_md_monitoring_outstanding extends Honda_Controller {
    ...
    public function laporan(){
        $data['isi']            = $this->page;
        $data['title']          = "Laporan Monitoring Outstanding";
        $data['set']            = "laporan";
        $this->template($data);
    }
    ...
    public function downloadReport(){
        ...
        $data['report'] = $this->db->query("
            select 
                IFNULL(psp.no_po,'-') as id_purchase_order,psp.id_part_int,psp.id_part,p.nama_part,
                date_format(ps.packing_sheet_date, '%d/%m/%Y') as packing_sheet_date,ps.packing_sheet_number,
                ps.id as packing_sheet_number_int,psp.no_doos,psp.packing_sheet_quantity,psp.qty_order as qty_order
                FROM tr_h3_md_ps ps
                join tr_h3_md_ps_parts psp on ps.id = psp.packing_sheet_number_int
                JOIN  tr_h3_dealer_purchase_order po on po.id = psp.no_po_int
                JOIN ms_part p on p.id_part_int = psp.id_part_int
                WHERE po.created_at  >= '$start_date 00:00:00' and po.created_at  <='$end_date 23:59:59'
        ");
        
                // WHERE po.created_at  >= '2022-03-01 00:00:00' and po.created_at  <='2023-10-01 23:59:59'
        $this->load->view("h3/laporan/temp_h3_md_monitoring_outstanding", $data);
    }
    //! end
