...
class H3_md_create_faktur extends Honda_Controller
{
	...
	function settingPDF()
	{
		...
		$data['namefile']= $data['parts'][0]['id_do_sales_order'].'.pdf';
		// var_dump($data['parts'][0]['id_do_sales_order']);
		// die;
		return $data;
	}
	...
	public function sendEmail()
	{
		...
		$config = array(
			...
			'smtp_port' => 465,
			'smtp_user' => 'dev.pebri@gmail.com',
			'smtp_pass' => 'qtkukrgmvkmfmxqk',
			'mailtype' => 'html',
			'charset' => 'iso-8859-1',
			'wordwrap' => TRUE
		);
		...
		$sendTo= 'tiaprila15@gmail.com';
		$dataEmail['nomor_faktur']= $data['faktur']['no_faktur'];
		$subject="Faktur berhasil dibuat dengan nomor faktur ". $data['faktur']['no_faktur'].'.';
		$message= $this->load->view('h3/h3_md_email_create_faktur', $data, true);
		$this->email->set_newline("\r\n");
		$this->email->from('dev.pebri@gmail.com');
		$this->email->to($sendTo);
		// $this->email->to($sendTo);
		$this->email->subject($subject);
		$this->email->message($message);
		$this->email->attach($content, 'attachment', $data['namefile'], 'application/pdf');
		if ($this->email->send()) {
			$faktur = $this->db
				->select('ps.id')
				->select('ps.faktur_send_email')
				->from('tr_h3_md_do_sales_order as do')
				->join('tr_h3_md_picking_list as pl', 'pl.id_ref = do.id_do_sales_order')
				->join('tr_h3_md_packing_sheet as ps', 'ps.id_picking_list = pl.id_picking_list')
				->where('do.id_do_sales_order', $this->input->get('id'))
			...
			$this->db
				->set('ps.tgl_send_email_terakhir', date('Y-m-d H:i:s'))
				->set('ps.faktur_send_email	', $faktur['faktur_send_email']+1)
				->where('ps.id', $faktur['id'])
				->update('tr_h3_md_packing_sheet as ps');

			$this->session->set_flashdata('pesan', 'Faktur berhasil terkirim ke email '.$sendTo.'.');
