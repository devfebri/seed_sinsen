...
class Mutasi_gudang_detail extends CI_Controller
{
    public function index()
    {
        ...
        foreach ($this->db->get()->result_array() as $row) {
            // $row['action'] = '<a href="h3/h3_md_mutasi_gudang/detail?id_part='. $row['id_part'].'" class="btn btn-xs btn-flat btn-info">View</a>';
            $data[] = $row;
        }
        // var_dump($data);
        // die;
    ...
    public function make_query()
    {
        ...
        $this->db
            ...
            ->select('mg.id_part')
            ->select('p.nama_part')
            ->select('date_format(mg.created_at, "%d/%m/%Y %H:%i:%s") as tanggal')
            ...
            ->where('mg.id_part=', $this->input->post('header'))
            ->order_by('mg.tanggal','desc')
            ;
