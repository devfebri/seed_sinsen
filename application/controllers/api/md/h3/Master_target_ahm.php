...
class Master_target_ahm extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data = array();
        $index = 1;
        // var_dump($this->input->post('set'));
        // die;
        foreach ($this->db->get()->result_array() as $row) {
            $row['action'] = $this->load->view('additional/md/h3/action_index_h3_md_master_target_ahm', [
                'id' => $row['id'],
            ], true);
    ...
    public function make_query()
    {
        ...
        $this->db
            ...
            ->select('ts.end_date')
            ->select('ts.jenis_target')
            ->select('ts.target_global_sales_in')
            ->select('ts.target_global_sales_out')
