...
class M_h2_api extends CI_Model
{
  ...
  function getSoPart($filter = null)
  {
    ...
    //! febri: query dikirim ke view\dealer\nsc.php num 540
    
  }
  //! add by febri
  function getPoPart($booking_id_reference, $nomor_so){
      $query1=$this->db->select("pop.id_part, part.nama_part ,pop.kuantitas,pop.tot_harga_part, pop.harga_setelah_diskon")
      ->from("tr_h3_dealer_purchase_order_parts pop")
      ->join("tr_h3_dealer_purchase_order po", "po.po_id = pop.po_id")
      ->join("ms_part part", "part.id_part_int=pop.id_part_int")
      ->where("po.id_booking", $booking_id_reference);
        $query2 = $this->db->query("select id_part FROM tr_h3_dealer_sales_order_parts so WHERE so.nomor_so = '$nomor_so'");
        foreach ($query2->result() as $key => $row) {
          $query1->where_not_in("pop.id_part",$row->id_part);
        }
      $query=$query1->get();
    return $query;
  }
