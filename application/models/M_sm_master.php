...
class M_sm_master extends CI_Model
{
  ...
  function getKesediaanCustLCR($filter = NULL)
  {
    $where = 'WHERE active = 1';
    if (isset($filter['id'])) {
      if ($filter['id'] != '') {
    ...
    return $this->db->query("
      SELECT id, deskripsi, active
      FROM ms_h2_kesediaan_customer_lcr
      $where
      $order
      $limit
    ");
  ...
  function getHasilCekLCR($filter = NULL)
  {
    $where = 'WHERE active = 1';
    if (isset($filter['id'])) {
      if ($filter['id'] != '') {
    ...
    return $this->db->query("
      SELECT id, deskripsi, active
      FROM ms_h2_hasil_pengecekan_lcr
      $where
      $order
      $limit
    ");
  ...
  function getAlasanLCR($filter = NULL)
  {
