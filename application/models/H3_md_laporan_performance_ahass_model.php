<?php
	...
	class H3_md_laporan_performance_ahass_model extends CI_Model{
		...
		public function laporan_sales_by_channel_selling_price_oil_m($id_dealer,$start_date,$end_date){
			...
			IFNULL(SUM(CASE WHEN md.h1=1 and md.h2=1 and md.h3=1 then (dso.total) else 0 end),0) as h123,
		...
		//* cost price part
		public function laporan_sales_by_channel_target_part_m1_cost($id_dealer,$start_date,$end_date){
			$filter_dealer = '';
			if ($id_dealer!='all') {
			...
			$report = $this->db->query("SELECT ifnull(SUM(CASE WHEN md.h1=1 and md.h2=1 and md.h3=1 then tsp.cost_target_part else 0 end),0) as h123,
			ifnull(SUM(CASE WHEN md.h1=0 and md.h2=1 and md.h3=1 then tsp.cost_target_part else 0 end),0) as h23,
			ifnull(SUM(CASE WHEN md.h1=0 and md.h2=0 and md.h3=1 then tsp.cost_target_part else 0 end),0) as h3
			...
			WHERE DATE_FORMAT(ts.start_date,'%Y-%m')='$start_date_bulan_m1' 
			and DATE_FORMAT(ts.end_date,'%Y-%m')='$end_date_bulan_m1'  
			and ts.jenis_target_salesman='Parts' $filter_dealer");
			return $report;
		}
		public function laporan_sales_by_channel_target_part_m_cost($id_dealer, $start_date, $end_date){
			$filter_dealer = '';
          		if ($id_dealer!='all') {
           			$filter_dealer = " AND tsp.id_dealer='$id_dealer'";
         		}
		...
		//* selling price part
		public function laporan_sales_by_channel_target_part_m1_selling($id_dealer,$start_date,$end_date){
			$filter_dealer = '';
          		if ($id_dealer!='all') {
           			$filter_dealer = " AND tsp.id_dealer='$id_dealer'";
         		}
		...
		public function laporan_sales_by_channel_target_part_m_selling($id_dealer,$start_date,$end_date){
			$filter_dealer = '';
          		if ($id_dealer!='all') {
           			$filter_dealer = " AND tsp.id_dealer='$id_dealer'";
         		}
			...
			JOIN ms_h3_md_target_salesman ts on ts.id=tsp.id_target_salesman 
			JOIN ms_dealer md on md.id_dealer=tsp.id_dealer
			WHERE DATE_FORMAT(ts.start_date,'%Y-%m')='$start_date_bulan' 
			and DATE_FORMAT(ts.end_date,'%Y-%m')='$end_date_bulan'  
			and ts.jenis_target_salesman='Parts' $filter_dealer");

			return $report;
		}

		//* cost price oil editByFebri	
		public function laporan_sales_by_channel_target_oli_m1_cost($id_dealer,$start_date,$end_date)
		{
			$filter_dealer = '';
          		if ($id_dealer!='all') {
           			$filter_dealer = " AND so.id_dealer='$id_dealer'";
			...
			$report = $this->db->query("SELECT ifnull(SUM(CASE WHEN md.h1=1 and md.h2=1 and md.h3=1 then so.cost_total_amount else 0 end),0) as h123,
			ifnull(SUM(CASE WHEN md.h1=0 and md.h2=1 and md.h3=1 then so.cost_total_amount else 0 end),0) as h23,
			ifnull(SUM(CASE WHEN md.h1=0 and md.h2=0 and md.h3=1 then so.cost_total_amount else 0 end),0) as h3
			from ms_h3_md_target_salesman_oil so 
			join ms_h3_md_target_salesman ts on ts.id = so.id_target_salesman 
			JOIN ms_dealer md on md.id_dealer=so.id_dealer
			WHERE DATE_FORMAT(ts.start_date,'%Y-%m')='$start_date_bulan_m1' 
			and DATE_FORMAT(ts.end_date,'%Y-%m')='$end_date_bulan_m1'  and ts.jenis_target_salesman='Oil' $filter_dealer");
			return $report;
		}

		public function laporan_sales_by_channel_target_oli_m_cost($id_dealer, $start_date, $end_date)
		{
			$filter_dealer = '';
			if ($id_dealer != 'all') {
				$filter_dealer = " AND so.id_dealer='$id_dealer'";
		...
		//* selling price oil editByFebri
		public function laporan_sales_by_channel_target_oli_m1_selling($id_dealer,$start_date,$end_date){
		...
		public function laporan_sales_by_channel_target_oli_m_selling($id_dealer, $start_date, $end_date)
		{
			$filter_dealer = '';
			if ($id_dealer != 'all') {
				$filter_dealer = " AND so.id_dealer='$id_dealer'";
		...
		public function laporan_htl_dph_md($id_dealer,$start_date,$end_date){
			$filter_dealer = '';
			if ($id_dealer!='all') {
				$filter_dealer = " AND po.id_dealer='$id_dealer'";
			}

			// $report = $this->db->query("SELECT cus.nama_customer ,po.po_id,  md.nama_dealer,md.alamat,pop.id_part ,mp.nama_part , pop.kuantitas,
			// 		DATE_FORMAT(po.created_at, '%d/%m/%Y') as tgl_po_dealer, po.status , po.created_at ,
			// 		DATE_FORMAT(pop.eta_terlama ,'%d/%m/%Y') as etd_ahm_to_md  , pop.kuantitas,pop.id_part_int
			// 	from tr_h3_dealer_purchase_order po 
			// 	JOIN tr_h3_dealer_purchase_order_parts pop on pop.po_id_int=po.id 
			// 	JOIN ms_dealer md on md.id_dealer=po.id_dealer 
			// 	JOIN tr_h3_dealer_request_document rd on rd.id=po.id_booking_int 
			// 	JOIN ms_customer_h23 cus on cus.id_customer=rd.id_customer 
			// 	JOIN ms_part mp on mp.id_part_int = pop.id_part_int 
			// 	where po.created_at>='2022-07-13 00:00:00' and po.created_at <='2022-07-13 23:59:59'
			// 	-- AND po.po_id ='PO/HLO/00675/2108/00001'
			// 	AND po.status not in ('Canceled','Rejected') and po.order_to = 0 
			// 	GROUP by po.po_id , pop.id_part
			// 	ORDER BY po.created_at ASC");

			$report = $this->db->query("SELECT cus.nama_customer ,po.po_id,  md.nama_dealer,md.alamat,pop.id_part ,mp.nama_part , pop.kuantitas, 
				grp.qty,(pop.kuantitas-sum(grp.qty)) as bo,DATE_FORMAT(po.created_at, '%d/%m/%Y') as tgl_po_dealer, po.status , po.created_at ,
				DATE_FORMAT(pop.eta_terlama ,'%d/%m/%Y') as etd_ahm_to_md,
				(select GROUP_CONCAT(DATE_FORMAT(gr.created_at ,'%d/%m/%Y')) from tr_h3_dealer_good_receipt gr 
				join tr_h3_dealer_good_receipt_parts grp on grp.id_good_receipt=gr.id_good_receipt where gr.nomor_po=po.po_id and grp.id_part_int =pop.id_part_int 
				and grp.qty >0 GROUP by gr.nomor_po ) as tgl_dipenuhi,
				po.created_at 
					from tr_h3_dealer_purchase_order po 
					JOIN tr_h3_dealer_purchase_order_parts pop on pop.po_id_int=po.id 
					JOIN ms_dealer md on md.id_dealer=po.id_dealer 
					...
					and grp.id_part_int=pop.id_part_int 
					where po.created_at>='$start_date 00:00:00' and po.created_at <='$end_date 23:59:59'
					-- WHERE po.po_id ='PO/HLO/00675/2108/00001'
					AND po.status not in ('Canceled','Rejected') and po.order_to = 0 
					GROUP by po.po_id , pop.id_part
					HAVING bo = 0
					ORDER BY po.created_at ASC");
		...
		public function laporan_htl_order_ke_ahm($id_dealer, $start_date, $end_date)
		{
			$filter_dealer = '';
			if ($id_dealer != 'all') {
				$filter_dealer = " AND po.id_dealer='$id_dealer'";
			}
			$report = $this->db->query("SELECT pomd.id_purchase_order ,cus.nama_customer ,pomd.id ,pomd.referensi_po_hotline,po.id_dealer,md.nama_dealer , 
					md.alamat,pomd.id_purchase_order,pomdp.id_part,mp.nama_part,pomdp.qty_order, DATE_FORMAT(po.tanggal_po_md ,'%d/%m/%Y') as tanggal_po_md , DATE_FORMAT(po.tanggal_po_ahm,'%d/%m/%Y') as tanggal_po_ahm ,
					pomdp.etd,pomdp.eta,po.proses_ahm,DATE_FORMAT(pop.eta_terlama ,'%d/%m/%Y') as etd_ahm_to_md,DATE_FORMAT(pop.eta_tercepat ,'%d/%m/%Y') as eta_dealer, pomd.created_at ,
					DATE_FORMAT(opt.tgl_shipping_list_md ,'%d/%m/%Y') as tgl_shipping_md, pbi.no_penerimaan_barang,DATE_FORMAT( mps.packing_sheet_date ,'%d/%m/%Y') as packing_sheet_date,DATE_FORMAT(fdo.invoice_date,'%d/%m/%Y') as invoice_date 
					FROM tr_h3_md_purchase_order pomd 
					JOIN tr_h3_md_purchase_order_parts pomdp on pomd.id_purchase_order=pomdp.id_purchase_order 
					JOIN ms_part mp on mp.id_part=pomdp.id_part 
					JOIN tr_h3_dealer_purchase_order po on po.po_id = pomd.referensi_po_hotline 
					JOIN ms_dealer md on po.id_dealer=md.id_dealer 
					JOIN tr_h3_dealer_request_document rd on rd.id=po.id_booking_int
					JOIN ms_customer_h23 cus on cus.id_customer=rd.id_customer 
					join tr_h3_dealer_purchase_order_parts pop on pop.id_part_int=pomdp.id_part_int and pop.po_id=pomd.referensi_po_hotline 
					JOIN tr_h3_dealer_order_parts_tracking opt on opt.po_id=pop.po_id and pop.id_part_int=opt.id_part_int 
					join tr_h3_md_penerimaan_barang_items pbi on pbi.no_po=pomd.id_purchase_order and pbi.id_part=pomdp.id_part
					join tr_h3_md_ps mps on pbi.packing_sheet_number = mps.packing_sheet_number
					join tr_h3_md_fdo fdo on fdo.id = mps.invoice_number_int  
					WHERE pomd.status IN ('Approved','Closed') and po.order_to = 0  $filter_dealer 
					AND pomd.created_at>='$start_date 00:00:00' and pomd.created_at <='$end_date 23:59:59'
					ORDER by pomd.id");

			return $report;
		}
		public function laporan_bo_htl_monitoring($id_dealer,$start_date,$end_date){
			$filter_dealer = '';
			if ($id_dealer!='all') {
				 $filter_dealer = " AND po.id_dealer='$id_dealer'";
		   }

			$report = $this->db->query("SELECT po.po_id, md.id_dealer,md.nama_dealer ,md.alamat, pomd.id_purchase_order ,pomdp.id_part,mp.nama_part,pomdp.qty_order, 
			DATE_FORMAT(po.tanggal_po_md ,'%d/%m/%Y') as tanggal_po_md , DATE_FORMAT(po.tanggal_po_ahm,'%d/%m/%Y') as tanggal_po_ahm, 
						DATE_FORMAT(pop.eta_terlama ,'%d/%m/%Y') as etd_ahm_to_md,pop.kuantitas,sum(grp.qty), (pop.kuantitas-sum(grp.qty)) as bo
						FROM tr_h3_md_purchase_order pomd 
						JOIN tr_h3_md_purchase_order_parts pomdp on pomd.id_purchase_order=pomdp.id_purchase_order 
						JOIN ms_part mp on mp.id_part=pomdp.id_part 
						JOIN tr_h3_dealer_purchase_order po on po.po_id = pomd.referensi_po_hotline 
						...
						join tr_h3_dealer_purchase_order_parts pop on pop.id_part_int=pomdp.id_part_int and pop.po_id=pomd.referensi_po_hotline 
						JOIN tr_h3_dealer_good_receipt gr on gr.nomor_po=po.po_id
						join tr_h3_dealer_good_receipt_parts grp on grp.id_good_receipt = gr.id_good_receipt and grp.id_part_int=pop.id_part_int 
						WHERE pomd.status IN ('Approved','Closed') and po.order_to = 0 
						AND pomd.created_at>='$start_date 00:00:00' and pomd.created_at <='$end_date 23:59:59'
						GROUP by po.po_id , pop.id_part
						HAVING bo > 0
						ORDER by pomd.id
